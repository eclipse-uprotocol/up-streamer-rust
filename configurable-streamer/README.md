...
## Issue #74 runbook (left-side/topic authority enforcement)

This runbook validates that publish forwarding honors the left side of `subscription_data.json`.
No source edits are required; use CLI overrides on example binaries.
For this issue, MQTT<->Zenoh is the canonical validation path; SOME/IP CLI parameterization is compatibility coverage and does not block issue #74 verification.

### 1) Prepare issue-specific configs under `$OPENCODE_CONFIG_DIR/reports/issue-74`

Set helpers from repo root:

```bash
REPO_ROOT=$(pwd)
ISSUE74_DIR="$OPENCODE_CONFIG_DIR/reports/issue-74"
mkdir -p "$ISSUE74_DIR"
```

`$ISSUE74_DIR/subscription_data.issue74.json`:
...
`$ISSUE74_DIR/CONFIG.issue74.json5`:
...
```json5
{
  ...
  usubscription_config: {
    file_path: "<ABS_PATH_TO_REPORT_DIR>/subscription_data.issue74.json",
  },
  transports: {
    zenoh: {
      config_file: "<ABS_PATH_TO_REPO>/configurable-streamer/ZENOH_CONFIG.json5",
      endpoints: [
        {
          authority: "authority-b",
    ...
    mqtt: {
      config_file: "<ABS_PATH_TO_REPO>/configurable-streamer/MQTT_CONFIG.json5",
      endpoints: [
        {
...
### 2) Start broker and streamer

```bash
docker compose -f utils/mosquitto/docker-compose.yaml up -d
cargo run -p configurable-streamer -- --config "$ISSUE74_DIR/CONFIG.issue74.json5"
```

### 3) Allowed source check (`authority-a`)

Subscriber (terminal A):

```bash
cargo run -p example-streamer-uses --bin zenoh_subscriber --features zenoh-transport -- --uauthority authority-b --uentity 0x5678 --uversion 0x1 --resource 0x1234 --source-authority authority-a --source-uentity 0x5BA0 --source-uversion 0x1 --source-resource 0x8001
```

Publisher A (terminal B):
...
Expected: subscriber logs repeated `PublishReceiver: Received a message` entries sourced from `authority-a`.

### 4) Disallowed source check (`authority-c`)

Subscriber (terminal C):

```bash
cargo run -p example-streamer-uses --bin zenoh_subscriber --features zenoh-transport -- --uauthority authority-b --uentity 0x5678 --uversion 0x1 --resource 0x1234 --source-authority authority-c --source-uentity 0x5BA0 --source-uversion 0x1 --source-resource 0x8001
```

Publisher C (terminal D):

```bash
cargo run -p example-streamer-uses --bin mqtt_publisher --features mqtt-transport -- --uauthority authority-c --uentity 0x5BA0 --uversion 0x1 --resource 0x8001
```

Expected:

- Subscriber C-source run shows no `PublishReceiver: Received a message` lines.
- Streamer logs include a left-side mismatch skip message for `in_authority='authority-c'` and topic authority `authority-a`.
